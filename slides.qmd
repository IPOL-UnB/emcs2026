...
## Introdução {background-color="#34495e"}

### Quem somos?

![Frederico, Gabriel e Igor Tobias](imgs/equipe.png)

### Antes de tudo - Instalação
...
Se os objetos são os "substantivos" do R (as coisas que armazenamos), as **funções** são os "verbos" (as ações que executamos sobre essas coisas). 

Toda função no R é identificada por um nome seguido de parênteses `()`. Dentro dos parênteses, nós inserimos os **argumentos**, que são as instruções ou os objetos que a função vai processar.

```{r}
# Criando um objeto numérico
meu_numero <- 81

# Usando a função sqrt() para calcular a raiz quadrada do objeto
sqrt(meu_numero)
```
...
O R base é extremamente poderoso, mas pode ser inconsistente. Para resolver isso, adotamos o **tidyverse**: um conjunto de pacotes para ciência de dados que compartilham a mesma filosofia de design, gramática e estrutura.

A "filosofia *tidy*" baseia-se em princípios claros:

- **Foco no ser humano:** O código deve ser tão fácil de ler quanto de escrever.
- **Consistência:** As funções compartilham a mesma estrutura (o primeiro argumento é sempre a base de dados).
- **Dados estruturados (tidy):** Tudo entra e sai no formato tabular (*data frames* ou *tibbles*).
- **Encadeamento:** As operações são lidas passo a passo, da esquerda para a direita, utilizando o pipe (`|>`).

### Nossa abordagem no curso

Para gerenciar o ecossistema e outros pacotes, substituiremos o uso tradicional de `install.packages()` e `library()` por uma solução mais eficiente voltada à reprodutibilidade.
...
[https://github.com/IPOL-UnB/emcs2026/blob/main/dados/municipios_br.csv](https://github.com/IPOL-UnB/emcs2026/blob/main/dados/municipios_br.csv)
...
O tidyverse é um conjunto de pacotes que compartilham princípios do manifesto *tidy*: [Tidy Tools Manifesto](https://cran.r-project.org/web/packages/tidyverse/vignettes/manifesto.html).

Manuais Tidyverse: [R4DS](https://r4ds.hadley.nz/) | [R4DS - Português](https://pt.r4ds.hadley.nz/)

### Formato tidy

Hadley Wickham <https://pt.r4ds.hadley.nz/data-tidy.html>
...
### `select()` — variações úteis {.smaller}
...
Aplicar a mesma transformação em dezenas de colunas uma a uma é ineficiente. A função `across()` permite aplicar operações a várias variáveis simultaneamente dentro de `mutate()` ou `summarise()`.

Ela exige dois argumentos principais:
1. **Quais colunas** afetar (usando `starts_with()`, `everything()`, etc.)
2. **Qual função** aplicar

### `across()` no `summarise()`

Podemos calcular a média de todas as colunas numéricas de uma só vez, agrupadas por região:
...
Agora que conhecemos o poder do `across()`, podemos usá-lo para inspecionar a qualidade da nossa base de dados. 

Para contar quantos valores ausentes (`NA`) existem em cada variável, aplicamos uma função anônima em todas as colunas simultaneamente:

```{r}
# everything() seleciona todas as colunas do dataset
# ~ sum(is.na(.x)) avalia se o dado é NA (TRUE/FALSE) e soma os verdadeiros
